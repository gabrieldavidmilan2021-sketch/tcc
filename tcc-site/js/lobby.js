document.addEventListener("DOMContentLoaded", async () => {
  ...
  function atualizarZero() {
    ...
    portaProxima = [...document.querySelectorAll(".porta.liberada")].find(porta => {
  ...
  function animar() {
    ...
    if (andando) { x += dx; y += dy; atualizarZero(); }
    requestAnimationFrame(animar);
  }

  document.addEventListener("keydown", event => {
    const tecla = event.key.toLowerCase();
    if (["w", "a", "s", "d"].includes(tecla)) { event.preventDefault(); teclas.add(tecla); }
